...
proxy_controller::proxy_controller(const sandboxed_proxy_data& proxy_data)
...
Steinberg::tresult PLUGIN_API proxy_controller::initialize(Steinberg::FUnknown* context)
{
...
Steinberg::tresult PLUGIN_API proxy_controller::terminate()
{
    return EditControllerEx1::terminate();
}

Steinberg::tresult PLUGIN_API proxy_controller::setComponentState(Steinberg::IBStream* state)
{
...
Steinberg::IPlugView* PLUGIN_API proxy_controller::createView(Steinberg::FIDString name)
{
    return _proxy_data.get_sandboxed_controller()->createView(name);
}

Steinberg::tresult PLUGIN_API proxy_controller::setState(Steinberg::IBStream* state)
{
...
Steinberg::tresult PLUGIN_API proxy_controller::getState(Steinberg::IBStream* state)
{
...
Steinberg::tresult PLUGIN_API proxy_controller::setParamNormalized(Steinberg::Vst::ParamID id, Steinberg::Vst::ParamValue value)
{
...
Steinberg::Vst::ParamValue PLUGIN_API proxy_controller::getParamNormalized(Steinberg::Vst::ParamID id)
{
    Steinberg::FUnknownPtr<Steinberg::Vst::IEditController> _edit_controller(_proxy_data.get_sandboxed_controller());
    if (!_edit_controller) {
        std::cout << "Sandbox error: interface IEditController not implemented by sandboxed class" << std::endl;
...
Steinberg::tresult PLUGIN_API proxy_controller::setComponentHandler(Steinberg::Vst::IComponentHandler* hostHandler)
{
    _proxyHandler = new proxy_component_handler(hostHandler);
    if (!_proxyHandler) {

    }
    
    if (_proxy_data.get_sandboxed_controller()->setComponentHandler(_proxyHandler) != Steinberg::kResultOk) {
    }
