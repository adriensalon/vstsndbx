...
struct sandbox_processor : public Steinberg::Vst::AudioEffect {
    ...
    // + ptr to just created sandboxed processor
    sandbox_processor(const Steinberg::FUID& controller_uid)
    {
...
{
    // return (Steinberg::Vst::IAudioProcessor*)new sandbox_processor((original_plugin_data*)context);
    sandboxed_plugin_data* orig = static_cast<sandboxed_plugin_data*>(context);
    return (Steinberg::Vst::IAudioProcessor*)new sandbox_processor(orig->proxy_controller_uid); // pass controller UID here
}